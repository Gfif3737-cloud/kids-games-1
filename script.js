...
const memoryCharacterSelect = document.getElementById("memoryTheme");

const memoryThemes = {
  cartoons: ["Смешарики", "Фиксики", "Маша и Медведь", "Винни Пух"],
};
...
function startMemory() {
  const size = memorySelect.value; // "4x2" или "4x5"
  const theme = memoryCharacterSelect.value;

  let cols = 4;
  let rows = size === "4x2" ? 2 : 5;
...
function updateStationList() {
  ...
  metroLines[line].forEach(st => {
    ...
    div.innerHTML = `${done ? "✅" : "⬜"} ${st}`;
    stationList.appendChild(div);
  });
}

/* ---- генерация без совпадений ---- */
