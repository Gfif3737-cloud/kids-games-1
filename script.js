...
const characters = [
  { id: "pin", name: "Пин", img: "img/pin.png" },
  { id: "bibi", name: "Биби", img: "img/bibi.png" },
  { id: "krosh", name: "Крош", img: "img/krosh.png" },
  { id: "losyash", name: "Лосяш", img: "img/losyash.png" }
];
...
function renderCharacterSelect(containerId) {
  ...
  characters.forEach((ch, i) => {
    ...
    btn.innerHTML = `<img src="${ch.img}" width="32"><br>${ch.name}`;
...
function memoryFlip(card, value) {
  ...
  card.innerHTML = `<img src="${selectedCharacter.img}">`;
  ...
  mazeData.forEach((row, y) => {
    row.forEach((cell, x) => {
      ...
      if (x === player.x && y === player.y) {
        ...
        d.innerHTML = `<img src="${selectedCharacter.img}">`;
      }
      box.appendChild(d);
    });
  });
