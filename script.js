...
/* --- ПРОСТОЙ И НАДЁЖНЫЙ ЛАБИРИНТ С ГАРАНТИРОВАННЫМ ВЫХОДОМ --- */
function generateMazeGame() {
  maze = createSimpleMaze(mazeSize);
  player = { x: 1, y: 1 };
  finish = { x: mazeSize - 2, y: mazeSize - 2 };
  maze[finish.y][finish.x] = 2;
  drawMazeGame();
}

function createSimpleMaze(size) {
  const grid = [];
