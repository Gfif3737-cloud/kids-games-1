...
// Первые 10 линий метро (по 5 на уровень)
const metroLines = [
...
function startMatch3Level() {
  ...
  const stationPool = lines.flatMap(l => l.stations);
  ...
  game.innerHTML = `
    ...
    <p>Уровень ${match3Level + 1} — собери станции</p>
    ...
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        if (remove[y][x]) {
          ...
          if (station) collectedStations.push(station);
          grid[y][x] = null;
        }
      }
    }
  ...
  // Условие победы — все станции уровня собраны
...
function showFullMetroMap() {
  clearGame();
  game.innerHTML = `
    <h2>🎉 Ты собрал карту метро!</h2>
    <p>${collectedStations.join(" • ")}</p>
  `;
}