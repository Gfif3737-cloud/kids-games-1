document.addEventListener("DOMContentLoaded", () => {
    ...
    function generateMazeWithGuaranteedPath() {
        ...
        // Выбираем случайную цель (не у края)
        const possibleGoals = [];
    ...
    function ensurePathExists() {
        ...
        // Если пути нет — прорубаем
        let px = 0, py = 0;
        for (let y = 0; y < size; y++) {
            for (let x = 0; x < size; x++) {
                if (visited[y][x] && (Math.abs(x - goal.x) + Math.abs(y - goal.y) < Math.abs(px - goal.x) + Math.abs(py - goal.y) || (px === 0 && py === 0))) {
                    px = x;
                    py = y;
                }
            }
