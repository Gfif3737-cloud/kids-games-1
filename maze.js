...
function generateMaze(level) {
  ...
  do {
    ...
    } while (startPos.x % 2 !== 0 || startPos.y % 2 !== 0); // Алгоритм работает только с чётными
    
    player = startPos;
    carve(player.x, player.y);
    
    // Выбираем случайную позицию для финиша (не рядом со стартом)
    do {
      goal = getRandomPosition();
    } while ((Math.abs(goal.x - player.x) < 3 && Math.abs(goal.y - player.y) < 3) || 
