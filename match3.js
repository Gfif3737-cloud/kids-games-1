document.addEventListener("DOMContentLoaded", () => {
    ...
    // Проверка, есть ли на поле готовые тройки
    ...
    function createBoard() {
        ...
        } while (hasAnyMatches()); // Перемешиваем, пока не исчезнут все тройки
    }
    ...
    function selectSquare(square) {
        if (!firstSelected) {
            ...
            if (isAdjacent) {
                ...
                // Если после свапа нет троек — меняем обратно
                setTimeout(() => {
                    if (!checkMatches()) {
                        swapSquares(firstSelected, square);
                    }
    ...
    function dropSquares() {
        ...
        // После падения убираем ВСЕ новые тройки БЕЗ начисления очков
        setTimeout(() => {
            while (checkMatches(true)) {} // true = silent mode (без очков и станций)
            
            // Финальная проверка, что поле снова без троек
            if (hasAnyMatches()) {
                // Если вдруг остались — убираем их принудительно
                removeAllMatchesSilent();
            }
        }, 200);
    }

    // Полностью очищает поле от троек (без начисления)
    function removeAllMatchesSilent() {
