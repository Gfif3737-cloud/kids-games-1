document.addEventListener("DOMContentLoaded", () => {
    ...
    // 5 ЦВЕТОВ ЛИНИЙ МЕТРО (синий и голубой теперь разные!)
    const lineColors = {
        sokolnicheskaya: "#e74c3c",   // 🔴 красная
        zamoskvoretskaya: "#2ecc71",  // 🟢 зелёная
        arbatsko_pokrovskaya: "#0033CC", // 🔵 тёмно-синяя
        filyovskaya: "#66CCFF",        // 💙 ярко-голубая
    ...
    function fixTriple(i1, i2, i3) {
        const otherColors = Object.values(lineColors).filter(c => c !== squares[i1].dataset.color);
    ...
    // Убираем все тройки с поля (без удаления клеток)
    ...
    function selectSquare(square) {
        if (!firstSelected) {
            ...
            if (isAdjacent) {
                ...
                setTimeout(() => {
                    if (hasAnyMatches()) {
                        // Если тройка есть — отмечаем станцию и даём очки
                        ...
                        // НЕ удаляем тройку! Она остаётся на поле
                    } else {
    ...
    // Найти цвет любой тройки на поле
