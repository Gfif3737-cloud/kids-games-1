...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==================== ЗАГРУЗКА СОБРАННЫХ СТАНЦИЙ ====================
    function loadCollectedStations() {
        return JSON.parse(localStorage.getItem('match3_collected')) || {};
    }
    ...
    let collectedStations = loadCollectedStations();
    ...
    function dropCells() {
        ...
        for (let col = 0; col < BOARD_WIDTH; col++) {
            ...
            // Записываем обратно снизу вверх
            for (let row = BOARD_WIDTH - 1; row >= 0; row--) {
                const idx = row * BOARD_WIDTH + col;
                board[idx].color = columnColors[BOARD_WIDTH - 1 - row];
            }
        }

        applyBoardColors();
        
        // Проверяем, есть ли ещё тройки после падения
    ...
    function removeAllMatches() {
        ...
        do {
            ...
            if (matches.length > 0) {
                ...
                applyBoardColors();
                dropCellsWithoutCheck();
            }
        } while (anyRemoved);
        isProcessing = false;
    }

    function dropCellsWithoutCheck() {
        for (let col = 0; col < BOARD_WIDTH; col++) {
            const columnColors = [];
            for (let row = BOARD_WIDTH - 1; row >= 0; row--) {
                const idx = row * BOARD_WIDTH + col;
    ...
    restartBtn.addEventListener('click', () => {
        // При нажатии "Заново" сбрасываем ТОЛЬКО доску, но НЕ станции
        initBoard();
        lineScore = 0;
        lineScoreElement.textContent = lineScore;
    });

    // Магазин бонусов (только бомба и ракета)
