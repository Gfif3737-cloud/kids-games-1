document.addEventListener('DOMContentLoaded', () => {
    ...
    // Прогресс уровней (открытие по очереди)
    ...
    // Завершение уровня и выдача награды
    ...
    function dropCells() {
        for (let col = 0; col < BOARD_WIDTH; col++) {
            ...
            // Создаём новые цвета для пустых клеток
            ...
            // Новый столбец: новые цвета сверху, старые снизу
    ...
    function handleClick(index) {
        if (selectedCell === null) {
            ...
            if (index !== selectedCell) {
                // Проверяем, являются ли клетки соседними (НЕ по диагонали!)
                ...
                // Соседство только по горизонтали или вертикали
                const isAdjacent = (
                    (Math.abs(x1 - x2) === 1 && y1 === y2) || // горизонталь
                    (Math.abs(y1 - y2) === 1 && x1 === x2)    // вертикаль
                );

                if (!isAdjacent) {
                    // Если не соседи - просто сбрасываем выделение
                    selectedCell = null;
                    return;
                }
                ...
                // Проверяем, есть ли тройки
                if (findMatches().length > 0) {
                    // Есть тройки - удаляем и проверяем снова
                    while (removeMatches(true)) {}
                } else {
                    // Нет троек - меняем обратно
