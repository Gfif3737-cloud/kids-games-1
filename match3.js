document.addEventListener("DOMContentLoaded", () => {
    ...
    let lineScore = 0;
    let totalScore = 0;
    let isDropping = false; // Флаг, чтобы не было конфликтов
    ...
    function selectSquare(square) {
        if (isDropping) return; // Блокируем клики во время падения
        ...
        if (isAdjacent) {
            ...
            if (hasAnyMatches()) {
                // Запускаем каскадный сбор троек
                processMatches();
            } else {
                swapSquares(firstSelected, square);
            }
    ...
    function processMatches() {
        ...
        // Собираем все тройки на поле
        do {
            matchesFound = false;
            
            // Собираем очки за тройки
            for (let i = 0; i < width * width; i++) {
                if (i % width <= width - 3) {
                    const color = squares[i].dataset.color;
                    if (color && squares[i + 1].dataset.color === color && squares[i + 2].dataset.color === color) {
    ...
    function dropSquares() {
        ...
        // Проверяем, появились ли новые тройки
        setTimeout(() => {
            isDropping = false;
            
            if (hasAnyMatches()) {
                // Если есть новые тройки — запускаем их сбор
                processMatches();
            } else {
                // Если троек нет — гарантируем, что будет хотя бы одна для игрока
                ensureAtLeastOneMatch();
            }
        }, 200);
    }

    // Функция, гарантирующая наличие хотя бы одной тройки на поле
    function ensureAtLeastOneMatch() {
        if (hasAnyMatches()) return;
        
        // Пробуем создать тройку обменом соседних клеток
        for (let i = 0; i < squares.length; i++) {
            if (i % width < width - 1) {
                // Пробуем горизонтальный обмен
                swapSquares(squares[i], squares[i + 1]);
                if (hasAnyMatches()) {
                    swapSquares(squares[i], squares[i + 1]); // Возвращаем обратно
                    return; // Тройка есть, оставляем как есть
                } else {
                    swapSquares(squares[i], squares[i + 1]); // Возвращаем обратно
                }
            }
            ...
            if (i < width * (width - 1)) {
                // Пробуем вертикальный обмен
                swapSquares(squares[i], squares[i + width]);
                if (hasAnyMatches()) {
                    swapSquares(squares[i], squares[i + width]); // Возвращаем обратно
                    return;
                } else {
                    swapSquares(squares[i], squares[i + width]); // Возвращаем обратно
                }
        ...
        // Если ничего не вышло — меняем несколько клеток случайно
