document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateLinesStatusUI() {
        ...
        if (completedLinesCount === level.lines.length && !userProgress[currentLevel]?.completed) {
            ...
            // Даём артефакт
            const artifactNames = ['Вагон А', 'Вагон Б', 'Вагон Г', 'Вагон Д'];
            const levelIndex = { level1: 0, level2: 1, level3: 2, level4: 3 }[currentLevel];
            alert(`🏛️ Артефакт: ${artifactNames[levelIndex]}`);
        }
    }
    ...
    function dropCells() {
        for (let col = 0; col < BOARD_WIDTH; col++) {
            // Собираем цвета в столбце снизу вверх
            const columnColors = [];
        ...
        if (selectedCell === null) {
            ...
            if (index !== selectedCell) {
                ...
                if (findMatches().length > 0) {
                    // Есть тройки - удаляем и продолжаем
