document.addEventListener("DOMContentLoaded", () => {
    ...
    function buyBonus(bonusType, cost) {
        ...
        switch(bonusType) {
            ...
            case "rocket":
                bonusMode = "rocket";
                alert("✨ Выбери клетку для ракеты");
                break;
        }
    }
    ...
    function shuffleBoard() {
        ...
        // Проверяем, есть ли тройки после перемешивания
        if (hasAnyMatches()) {
    ...
    function useRocket(index) {
        const y = Math.floor(index / width); // Получаем номер строки
        
        // Удаляем всю строку (горизонталь)
    ...
    // ИСПРАВЛЕННАЯ ФУНКЦИЯ УДАЛЕНИЯ ТРОЕК
    function removeMatchesAndDrop() {
        let anyRemoved = false;
        
        // Проверяем горизонтальные тройки
        for (let row = 0; row < width; row++) {
            for (let col = 0; col < width - 2; col++) {
                const index = row * width + col;
                const color = squares[index]?.dataset.color;
                
                if (color && 
                    squares[index + 1]?.dataset.color === color && 
                    squares[index + 2]?.dataset.color === color) {
                    
                    // Удаляем тройку
                    squares[index].dataset.color = "";
                    squares[index + 1].dataset.color = "";
                    squares[index + 2].dataset.color = "";
                    squares[index].style.backgroundColor = "transparent";
                    squares[index + 1].style.backgroundColor = "transparent";
                    squares[index + 2].style.backgroundColor = "transparent";
                    anyRemoved = true;
                }
            }
        }
        
        // Проверяем вертикальные тройки
        for (let col = 0; col < width; col++) {
            for (let row = 0; row < width - 2; row++) {
                ...
                if (color && 
                    squares[index + width]?.dataset.color === color && 
                    squares[index + width * 2]?.dataset.color === color) {
                    
                    // Удаляем тройку
                    squares[index].dataset.color = "";
                    squares[index + width].dataset.color = "";
                    squares[index + width * 2].dataset.color = "";
                    squares[index].style.backgroundColor = "transparent";
                    squares[index + width].style.backgroundColor = "transparent";
                    squares[index + width * 2].style.backgroundColor = "transparent";
                    anyRemoved = true;
                }
            }
        }
