document.addEventListener("DOMContentLoaded", () => {
    ...
    function checkAllLevelsAchievement() {
        if (!achievements.allLevels) {
            ...
            if (allCompleted) {
                ...
                const achievementText = `🏆 Все уровни три в ряд пройдены!`;
                if (!artifacts.match3Achievements.includes(achievementText)) {
                    artifacts.match3Achievements.push(achievementText);
                    localStorage.setItem('metro_artifacts', JSON.stringify(artifacts));
    ...
    function checkScoreAchievements(score) {
        const scoreMilestones = [500, 1000, 1500, 2000, 2500];
        const scoreNames = ['500', '1000', '1500', '2000', '2500'];
        
        for (let i = 0; i < scoreMilestones.length; i++) {
            if (score >= scoreMilestones[i] && !achievements[`score${scoreMilestones[i]}`]) {
                achievements[`score${scoreMilestones[i]}`] = true;
                if (!artifacts.match3Achievements) artifacts.match3Achievements = [];
                const achievementText = `🎯 ${scoreMilestones[i]} очков за одну игру!`;
                if (!artifacts.match3Achievements.includes(achievementText)) {
                    artifacts.match3Achievements.push(achievementText);
                    localStorage.setItem('metro_artifacts', JSON.stringify(artifacts));
                    setTimeout(() => alert(`🏅 ДОСТИЖЕНИЕ!\n\n${achievementText}`), 500);
    ...
    function checkScoreWithoutCompletion() {
        if (!achievements.scoreWithoutCompletion && bestScoreThisRun > 0) {
            if (!userProgress[currentLevel]?.completed) {
                ...
                if (!artifacts.match3Achievements) artifacts.match3Achievements = [];
                const achievementText = `🎲 Мастер комбинаций! ${bestScoreThisRun} очков без завершения уровня!`;
                if (!artifacts.match3Achievements.includes(achievementText)) {
                    artifacts.match3Achievements.push(achievementText);
                    localStorage.setItem('metro_artifacts', JSON.stringify(artifacts));
                    setTimeout(() => alert(`🏅 ДОСТИЖЕНИЕ!\n\n${achievementText}`), 500);
