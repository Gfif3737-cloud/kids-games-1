document.addEventListener('DOMContentLoaded', () => {
    ...
    function dropCells() {
        for (let col = 0; col < BOARD_WIDTH; col++) {
            // Собираем ВСЕ цвета в столбце (включая пустые)
            const columnColors = [];
            for (let row = 0; row < BOARD_WIDTH; row++) {
            ...
            // Убираем пустые (удалённые клетки)
            const nonEmptyColors = columnColors.filter(color => color !== '');
            
            // Сколько пустых клеток было удалено
            ...
            // Новый столбец: сначала новые цвета (сверху), потом старые (снизу)
            const newColumn = [...newColors, ...nonEmptyColors];
            
            // Записываем обратно сверху вниз
            for (let row = 0; row < BOARD_WIDTH; row++) {
                const idx = row * BOARD_WIDTH + col;
