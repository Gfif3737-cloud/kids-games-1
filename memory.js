document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentLevel = 0; // индекс в LEVELS
    let board = [];
    let levelProgress = {}; // { theme: { level0: completed, level1: completed, ... } }
    ...
    function loadProgress() {
        ...
        if (saved) {
            ...
            for (let theme in THEMES) {
                ...
                levelProgress[theme][LEVELS[0].id] = true; // первый уровень открыт
            }
        }
    }
    ...
    // Получение доступных уровней для выбора
    ...
    function giveReward(levelIndex) {
        ...
        // Сохраняем награду в отдельную категорию
    ...
    function completeLevel() {
        ...
        if (!levelProgress[currentTheme][levelId + '_completed']) {
            ...
            setTimeout(() => {
                alert(`🎉 Уровень пройден! Получена награда!\n\nСледующий уровень: ${LEVELS[currentLevel + 1]?.name || 'все уровни пройдены!'}`);
            }, 300);
        }
    }
    ...
    function flipCard(index) {
        ...
        if (!firstCard) {
            ...
            if (board[firstCard].name === card.name) {
                // Совпадение
                board[firstCard].matched = true;
                board[index].matched = true;
                board[firstCard].open = false;
                card.open = false;
                board[firstCard].element.style.opacity = '0.6';
                ...
                setTimeout(() => {
                    board[firstCard].open = false;
                    board[index].open = false;
                    board[firstCard].element.style.backgroundColor = '#2c3e50';
                    board[index].element.style.backgroundColor = '#2c3e50';
                    board[firstCard].element.style.fontSize = '24px';
                    board[index].element.style.fontSize = '24px';
                    board[firstCard].element.textContent = '❓';
