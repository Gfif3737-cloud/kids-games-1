    ...
    function checkAchievements(theme, levelIndex, time) {
        ...
        if (!achievements.allDifficulties[theme]) {
            ...
            if (allCompleted) {
                ...
                const achievementText = `🌟 Все сложности пройдены в мультфильме "${themeName}"!`;
                if (!artifacts.memoryAchievements.includes(achievementText)) {
                    artifacts.memoryAchievements.push(achievementText);
                    localStorage.setItem('metro_artifacts', JSON.stringify(artifacts));
        ...
        if (!achievements.allLevels) {
            ...
            if (allThemesCompleted) {
                ...
                if (!artifacts.memoryAchievements) artifacts.memoryAchievements = [];
                const achievementText = `🏆 Полная коллекция! Все уровни во всех мультфильмах пройдены!`;
                if (!artifacts.memoryAchievements.includes(achievementText)) {
                    artifacts.memoryAchievements.push(achievementText);
                    localStorage.setItem('metro_artifacts', JSON.stringify(artifacts));
                    setTimeout(() => alert(`🏅 ДОСТИЖЕНИЕ!\n\n${achievementText}`), 500);
        ...
        if (level.size === '4x6' && time < 20) {
            if (!achievements.fastWin[theme]) {
                ...
                if (!artifacts.memoryAchievements) artifacts.memoryAchievements = [];
                const achievementText = `⚡ Скоростной рекорд! Уровень Мастер в "${themeName}" пройден за ${time} секунд!`;
                if (!artifacts.memoryAchievements.includes(achievementText)) {
                    artifacts.memoryAchievements.push(achievementText);
                    localStorage.setItem('metro_artifacts', JSON.stringify(artifacts));
                    setTimeout(() => alert(`🏅 ДОСТИЖЕНИЕ!\n\n${achievementText}`), 500);
